...
import org.jellyfin.mobile.update.UpdateManager.Companion.UPDATE_DIRECTORY_NAME
 ...
 *
 * Runs as a foreground worker so the download continues when the user leaves the app, mirroring
 * [org.jellyfin.mobile.downloads.DownloadWorker].
 */
class UpdateDownloadWorker(
    ...
    companion object {
        ...
        /** How often the progress notification and the state are updated. */
            ...
            val request = OneTimeWorkRequestBuilder<UpdateDownloadWorker>().apply {
                ...
                setInputData(
                    workDataOf(
                        ...
                        KEY_FILE_NAME to release.fileName,
                    ),
                )
            }.build()
    ...
    override suspend fun getForegroundInfo(): ForegroundInfo = updateNotificationManager.createForegroundInfo()
    ...
    private suspend fun download(url: String, target: File): ByteArray? {
        ...
        reportProgress(target.length(), target.length())
    ...
    private fun reportProgress(downloaded: Long, total: Long) {
        updateManager.onDownloadProgress(downloaded, total)
        val progress = if (total <= 0L) {
            0
        } else {
            ((downloaded * UpdateManager.PROGRESS_MAX) / total).toInt().coerceIn(0, UpdateManager.PROGRESS_MAX)
        }
        updateNotificationManager.updateProgress(release.version, progress)
    }
