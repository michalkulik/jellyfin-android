 ...
 *
 * Only plain OkHttp is used, like [org.jellyfin.mobile.downloads.DownloadJobClient], because the
 * manifest is hosted by GitHub and not by the Jellyfin server.
 */
class UpdateClient(
    ...
    /**
     * Fetches [manifestUrl] and returns the release for [variant] when it is usable on this device.
     *
    ...
    companion object {
        ...
        /**
         * The URL of the manifest that describes the newest release, which is the manifest attached
         * to the newest release of the fork.
         */
        val DEFAULT_MANIFEST_URL: String
            get() = "${Constants.UPDATE_RELEASES_URL}/latest/download/${Constants.UPDATE_MANIFEST_FILE}"
    }
}
