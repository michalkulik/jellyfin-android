...
/**
 * Parsed `latest.json`, published as an extra asset of every release next to the APKs.
 *
...
/**
 * A release that can actually be installed on this device, meaning the variant matching the flavour
 * of the running app is present in the manifest.
 */
data class UpdateRelease(
     ...
     */
    val fileName: String
        get() = "jellyfin-android-$versionCode-$variant.apk"

    companion object {
        /**
         * Name pattern used by [fileName], used to read the version code back from the disk.
         */
        val FILE_NAME_REGEX = Regex("""jellyfin-android-(\d+)-[a-z]+\.apk""")
    }
}

/**
 * Mirror of `buildSrc/src/main/kotlin/VersionUtils.kt` (MA.MI.PA-PR -> MAMIPAPR) so a manifest
 * without an explicit `versionCode` can still be compared with [org.jellyfin.mobile.BuildConfig].
 *
